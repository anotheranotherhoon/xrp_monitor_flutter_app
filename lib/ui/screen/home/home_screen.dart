...
import 'package:xrp_monitor/core/services/popup/popup_service.dart';
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: portfolioAsyncValue.when(
          data:
              (portfolioState) => StreamBuilder<ChartData>(
                ...
                builder: (context, snapshot) {
                  ...
                  // 데이터가 없는 상태
                  if (!snapshot.hasData) {
                    return Center(child: Text(AppStrings.waitingData));
                  }
                  ...
                  return Column(
                    children: [
                      ...
                      // 차트
                      Expanded(child: ChartGraph(chartData: chartData)),
                    ],
                  );
                },
              ),
          loading: () => const LoadingScreen(),
          error: (error, stack) => Center(child: Text('포트폴리오 로딩 오류: $error')),
        ),
      ),
    );
  }
